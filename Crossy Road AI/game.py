...
class Game:
    ...
    ****THE 6 GAME SCENES****
    ...
    def leaderboard(self):
        ...
        game_button, menu_button =  self.create_text("Play", 20, 200), self.create_text("Home", 20, 500)

        for i, top_player in enumerate(reversed(self.user.get_leaderboard())):
            background = self.create_textbox((153, 153, 255), 100, i * 70 + 100, 1000, 50)

            user_name_text = self.default_font.render(top_player[0], True, (0,0,0))
            score_text = self.default_font.render(str(top_player[1]), True, (0,0,0))
    ...
    ****THE END 4 GAME SCENES****
    ...
    def create_text(self, name, x, y):
        ...
        :param y: int
        """
        surface = self.default_font.render(name, True, (165,42,42), (255, 165, 0))
